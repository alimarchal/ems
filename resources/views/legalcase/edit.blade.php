<x-app-layout>
    ...
    <div>
        <div class="max-w-7xl mx-auto py-5">
            ...
            <x-alert/>
            <form action="{{route('legalCase.update', $legalCase->id)}}" method="POST" enctype="multipart/form-data">
                @csrf
                                            ...
                                            <div class="px-4 py-5 bg-white sm:p-6">
                                                <div class="grid grid-cols-6 gap-6">
                                                    <div class="col-span-6">
                                                        ...
                                                        <input type="text" name="case_title" id="case_title" value="{{$legalCase->case_title}}" autocomplete="case_title" required="required" class="border-gray-300 @error('case_title') border-red-500 @enderror mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm  rounded-md">
                                                        @error('case_title')<span class="text-red-500 mt-1 text-sm">{{ $message }}</span>@enderror
                                                    </div>

                                                    <div class="col-span-6">
                                                        <label for="description" class="block text-sm font-medium text-gray-700">Description</label>
                                                        <textarea id="description" name="description" required="required" rows="3" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 mt-1 block w-full sm:text-sm border border-gray-300 @error('description') border-red-500 @enderror rounded-md" placeholder="Case description...">{{$legalCase->description}}</textarea>
                                                    ...
                                                    <div class="col-span-6 sm:col-span-3">
                                                        ...
                                                        <select id="status" name="status"  autocomplete="nationality" class="mt-1 block w-full py-2 px-3 border border-gray-300 @error('status') border-red-500 @enderror bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                                            <option value="Sub Judice" @if( $legalCase->status == 'Sub Judice' ) selected="selected" @endif>Sub Judice</option>
                                                            <option value="Decided" @if( $legalCase->status == 'Decided' ) selected="selected" @endif>Decided</option>
                                                        </select>
